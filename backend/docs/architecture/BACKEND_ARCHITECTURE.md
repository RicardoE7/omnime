...
`frontend/`; Railway targets `backend/`; PostgreSQL is a separate
Railway service.
...
Spring Security CSRF protection remains enabled for authenticated cookie-based unsafe methods. The backend provides a CSRF token through Spring's cookie/token-repository mechanism; the CSRF token is readable by React while the authentication JWT remains HttpOnly. React echoes the CSRF token in the configured CSRF request header for POST/PUT/PATCH/DELETE requests. Requests missing or failing CSRF validation are rejected.

CORS is configured separately with the explicit frontend origin and credentials enabled; wildcard origins are not permitted with credentialed requests. The authentication cookie remains Secure in production and uses a SameSite/domain configuration appropriate to the final Vercel/Railway or custom-domain topology. Exact cookie/header names may remain implementation configuration. Integration tests must cover an authenticated unsafe request succeeding with a valid CSRF token and failing without/with an invalid token.
...
Railway backend environment configuration will conceptually include:
...
-   SameSite/domain behavior must match the final
    Vercel/Railway/custom-domain topology
-   CSRF protection must be intentionally designed
-   CORS must not be mistaken for CSRF protection

CSRF protection is required for authenticated cookie-based mutations. Spring Security CSRF protection remains enabled; the backend exposes a CSRF token that React returns in the required custom header. The authentication JWT remains HttpOnly. Exact Spring class/header/cookie names and SameSite/domain values remain deployment implementation details.
...
An external-provider outage should not cause Railway to repeatedly
...
- **CSRF:** Spring Security CSRF remains enabled for authenticated cookie mutations; React reads the separate CSRF token and echoes it in the configured header while the auth JWT remains HttpOnly. CORS is separate, explicit-origin, and credential-aware; integration tests cover success/failure.
...
-   Vercel frontend + Railway backend/database deployment
